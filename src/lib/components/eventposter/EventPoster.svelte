...
<div class="event-poster">
  <div class="header">
    ...
    <div class="event-meta">
      <p class="date">📅 {new Date(eventData.event.time_and_date.start).toLocaleDateString()} {new Date(eventData.event.time_and_date.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {new Date(eventData.event.time_and_date.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
      <p class="location">📍 {eventData.event.location}</p>
      <p class="ticket">🎟️ {eventData.event.ticket}</p>
    </div>
    <a href={eventData.event.meetlink} target="_blank" rel="noopener" class="join-button">
      Join
    </a>
  </div>
  ...
  <div class="tabs">
    ...
    <button class:active={activeTab === 'speakers'} on:click={() => setTab('speakers')}>Speakers</button>
  </div>
