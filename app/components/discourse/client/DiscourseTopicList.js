...
function DiscourseTopicList({
  ...
  const getAvatarUrl = (user, size = 32) => {
    if (!user?.avatar_template) return DefaultAvatarUrl;
  ...
  return (
    <>
      <Col className={`${styles.container} ${className}`}>
        {topics.slice(0, max).map((item, i) => (
          <Col
            key={item.id}
            className={`${styles.item} ${color[Math.floor(i % color.length)]}`}
          >
            <Row className={`${styles.item_container}`}>
              ...
              <div className={`fw-light ${styles.time}`}>
                updated{' '}
                {TimeSince(new Date(item.last_posted_at || item.created_at))}
              </div>
